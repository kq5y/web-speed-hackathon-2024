...
const getConstant = async (key: string): Promise<string> => {
  const response = await fetch(`/assets/text/${key}.txt`);
...
export const Footer: React.FC = () => {
  ...
  const handleRequestToTermDialogOpen = async () => {
    const TERM = await getConstant('term');
    updateDialogContent(
  ...
  const handleRequestToContactDialogOpen = async() => {
    const CONTACT = await getConstant('contact');
    updateDialogContent(
  ...
  const handleRequestToQuestionDialogOpen = async () => {
    const QUESTION = await getConstant('question');
    updateDialogContent(
  ...
  const handleRequestToCompanyDialogOpen = async () => {
    const COMPANY = await getConstant('company');
    updateDialogContent(
  ...
  const handleRequestToOverviewDialogOpen = async () => {
    const OVERVIEW = await getConstant('overview');
    updateDialogContent(
  ...
  return (
    <Box as="footer" backgroundColor={Color.Background} p={Space * 1}>
      <Flex align="flex-start" direction="column" gap={Space * 1} justify="flex-start">
        <img alt="Cyber TOON" src="/assets/img/cyber-toon.avif" width={189} height={45} loading="lazy" />
