...
type Props = {
  book: {
    id: GetBookListResponse[number]['id'];
    name: GetBookListResponse[number]['name'];
    description: GetBookListResponse[number]['description'];
    image: GetBookListResponse[number]['image'];
  };
};

export const BookListItem: React.FC<Props> = ({ book }) => {
  const imageUrl = useImageSrc({ height: 64, imageId: book.image.id, width: 64 });
